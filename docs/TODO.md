...
- [!] **win/arm64：实测会崩，缺口已复现**（#497）—— CI 的 `windows-arm64-run` 跑在**原生 windows-11-arm** 上，
      把外置产物的三形态验收挂上去后，三条**全部**以 `0xC0000005`(ACCESS_VIOLATION) 退出（与有无密钥无关），
...
- [ ] **win/arm64 续（诊断第 2 轮）**：先**校准打印器**（在更早、必然执行的阶段打标记，如镜像自解密入口）——
      ...
      再往上游二分（入口自解密 → 描述符/掩码 → 蹦床）。诊断代码与复现器已在库（STATUS #498）。

- [ ] **win/arm64 续（第 3 轮）**：先做**宏探针** —— 把 `vm_interp.c` 里那条 `#error` 拆成多条、逐条报出
      哪一组宏成立（`__x86_64__` / `_M_X64` / `__aarch64__` / `_M_ARM64` / `VM_HOST_X86_32`），
      一次 CI 往返确定该工具链的真实宏集合，再据此修正 `VM_ARCH_AARCH64` 的判定。
      校准用例已就位：`tools/e2e_win_arm64.ps1` 会先用非外置产物校准 harness（必须与 native 一致）。

- [ ] **win/arm64 续（第 4 轮）**：崩溃已消除、取钥已执行、硬门正确；剩余是产物以 `0xC0DE0002`
      （`vm_img_fail(2)`：需要重定位但没有重定位表）失败，**非外置产物同样如此** ⇒ 与密钥无关。
      两边 vmpbuild/vmpack 参数已逐字比对一致 ⇒ 下一步应查"**我的流程与绿作业的最后差异**"
      （产物文件名/扩展名、运行顺序、是否残留旧的 `.vmpkey`）以及入口自解密在 Windows/ARM64 上的重定位判定。

- [ ] **win/arm64 续（第 17 轮起）第一件事**：解开"CI 里探针一行都不打印、而它之后的校准却打印"的谜题 ——
      探针已改成无条件，本地文件里它**在校准之前**，日志却完全没有它。候选：checkout 的行尾转换、
      脚本被别处覆盖、`-File` 调用路径/工作目录差异。解不开就无法回答 `0xC0DE0002` 的归属。

- [ ] **win/arm64 续（新目标第 1 轮起）**：把探针改成**文件信号**（脚本 `Add-Content build/probe.txt`，
      CI 步骤 `Get-Content`）—— 已确认部署脚本含无条件探针（第 73 行、run 35963405531）且控制流经过它，
      但输出始终不出现 ⇒ 只剩"输出被吞"这一解释。拿到探针结果后才能回答 `0xC0DE0002` 的归属。

- [ ] **win/arm64 下一次实验**：在**绿作业自己的步骤里**（其 native vs protected 比对之后）再跑一次同一个产物
      并记录退出码 ⇒ 区分"先前那次成功另有原因"与"状态在步骤之间变化"。当前已知：同一产物在绿步骤成功、
      在下一步骤两种启动方式都失败；ASLR/我的重建/启动方式三个假设均已用 CI 实验排除（STATUS #505）。
