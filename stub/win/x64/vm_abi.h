...
#define VM_MARGIN       0x4000 /* 16KB（重试：验证 CI 的 mt 失败是否为偶发）原注释：3KB：CI 里 Go/线程栈的既有预算。放大到 16KB 能修好 add_dly，但 CI 的 mt(0) 多线程用例会挂
                              *（本地不复现），所以只有"私有客户机栈 + 运行期 skew"那条重构才是正解，见 docs/STATUS.md */ /* 3KB：CI 里 Go 的 goroutine 栈只有 4KB（日志 stack=[0xc000078000,0xc000079000]），总深度必须 < 4KB */
