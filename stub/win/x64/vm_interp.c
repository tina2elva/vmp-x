...
#endif
/* i686(Win32) 的 Win32/ntdll/bcrypt API 全是 __stdcall（callee 清栈）。若按默认 cdecl 调用，
 * 调用方会**再清一次**栈 ⇒ 宿主栈被逐次破坏（这是 32 位打包路径"值算错/栈守卫误报"的根因）。
 * x64 只有一种调用约定，__attribute__((stdcall)) 被忽略，故对 x64 无影响。 */
#if defined(VM_HOST_X86_32)
